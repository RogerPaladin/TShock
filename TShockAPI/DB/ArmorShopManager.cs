    ...
    public class ArmorShopManager
    {
        ...
        public List<string> InGameNames()
        {
            try
            {
                ...
                result = database.QueryReader("SELECT * FROM Armor");
        ...
        public bool GetArmor(string ReqName, out string Name, out string Contains, out double Price)
        {
            Price = 0;
            Name = string.Empty;
            ...
            try
            {
                using (var reader = database.QueryReader("SELECT * FROM Armor WHERE InGameName=@0", ReqName))
                {
                    if (reader.Read())
                    {
